...
AccelerometerSensor::AccelerometerSensor()
	: SensorBase(NULL, "lsm303dlh_acc_sysfs"),
		mEnabled(0),
		mPendingMask(0),
		mInputReader(32)
{
...
int AccelerometerSensor::enable(int32_t, int en) {
	...
	if (flags != mEnabled) {
		int fd;
		fd = open("/sys/devices/i2c-0/0-0018/enable_device", O_RDWR);
		if (fd >= 0) {
			char buf[2];
			buf[1] = 0;
			if (flags) {
...
int AccelerometerSensor::readEvents(sensors_event_t* data, int count)
{
	...
	while (count && mInputReader.readEvent(&event)) {
		...
		if (type == EV_ABS) {
			...
			LOGE("AccelerometerSensor: unknown event (type=%d, code=%d)",
				type, event->code);
			mInputReader.next();
		}
	}
...
{
    LOGD("AccelerometerSensor: processEvent(%d, %d) called", code, value);

    switch (code) {
        case EVENT_TYPE_ACCEL_X:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.x = value * CONVERT_A_X;
            LOGD("AccelerometerSensor: acceleration.x = %f", mPendingEvents[Accelerometer].acceleration.x);
            break;
        case EVENT_TYPE_ACCEL_Y:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.y = value * CONVERT_A_Y;
            LOGD("AccelerometerSensor: acceleration.y = %f", mPendingEvents[Accelerometer].acceleration.y);
            break;
        case EVENT_TYPE_ACCEL_Z:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.z = value * CONVERT_A_Z;
            LOGD("AccelerometerSensor: acceleration.z = %f", mPendingEvents[Accelerometer].acceleration.z);
            break;
        case EVENT_TYPE_YAW:
